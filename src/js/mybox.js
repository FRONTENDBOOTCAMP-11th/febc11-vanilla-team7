...
export function mybox() {
    // 공통으로 사용되는 색상 배열
    const cardColors = [
        'wheat', 'crimson', 'orangered', 'firebrick', 'lightsalmon',
        'orange', 'peru', 'khaki', 'yellowcreen', 'darksalmon',
        'tomato', 'bisque', 'maroon', 'sienna', 'forestgreen',
        'darkgreen', 'cadetblue', 'teal', 'aqua', 'steelblue',
        'lightsteelblue', 'darkslateblue', 'mediumpurple', 'plum'
    ];
    ...
    async function renderRecentPosts() {
        ...
        try {
            ...
            container.innerHTML = '<p class="text-gray-500">글을 불러오는데 실패했습니다.</p>';
        }
    }
    ...
    function renderInterestPosts() {
        ...
        getBookmarkedPosts()
            .then(data => {
                const likedPosts = data.item.filter(bookmark => 
                ...
                likedPosts.forEach(bookmark => {
                    ...
                    postNode.onclick = () => window.navigate('post', null, bookmark.post._id);
                    container.appendChild(postNode);
                });
            })
        ...
        getMyBrunchData()
            .then(data => {
                const myPosts = data.item
                    .filter(post => post.user._id === parseInt(userId) && post.user.name === userName)
                    .slice(0, 3);

                brunchSection.innerHTML = myPosts.map(post => `
